...
GPU와 VRAM은 시작할 때 기록하지만, 5060 Ti 16GB 실측에서 기본 정밀도도 충분히
들어갈 여유가 확인됐으므로 모든 GPU는 기존 FP16/BF16으로 시작합니다. FP8은 같은
seed에서도 그림체가 크게 달라질 수 있어 실제 OOM이 확인된 경우에만 사용합니다.

```bash
COMFYUI_PRECISION_MODE=fp8 chatos restart
```
